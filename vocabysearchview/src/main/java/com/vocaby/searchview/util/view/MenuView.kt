...
class MenuView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    LinearLayout(context, attrs) {
    private val HIDE_IF_ROOM_ITEMS_ANIM_DURATION = 400
    private val SHOW_IF_ROOM_ITEMS_ANIM_DURATION = 450
    private val ACTION_DIMENSION_PX: Float = context.resources.getDimension(R.dimen.square_button_size)
    private var mMenu = -1
    private lateinit var mMenuBuilder: MenuBuilder
    private lateinit var mMenuInflater: SupportMenuInflater
    private lateinit var mMenuPopupHelper: MenuPopupHelper
    private var mMenuCallback: MenuBuilder.Callback? = null
    private var mActionIconColor = 0
    private var mOverflowIconColor = 0

    //all menu items
    private lateinit var mMenuItems: MutableList<MenuItemImpl>
    ...
    val currentMenuItems: List<MenuItemImpl> get() = mMenuItems
    ...
    fun setActionIconColor(actionColor: Int) {
        mActionIconColor = actionColor
        refreshColors()
    }

    fun setOverflowColor(overflowColor: Int) {
        mOverflowIconColor = overflowColor
        refreshColors()
    }
    ...
    fun reset(menu: Int, availWidth: Int) {
        ...
        mMenuItems.sortWith { lhs, rhs -> lhs.order.compareTo(rhs.order) }
        ...
        if (mOnVisibleWidthChangedListener != null) {
            visibleWidth =
                ACTION_DIMENSION_PX.toInt() * childCount - if (mHasOverflow) Util.dpToPx(8) else 0
    ...
    private fun createActionView(): ImageView {
        return LayoutInflater.from(context)
            .inflate(R.layout.action_item_layout, this, false) as ImageView
    }

    private val overflowActionView: ImageView
        get() = LayoutInflater.from(context)
            .inflate(R.layout.overflow_action_item_layout, this, false) as ImageView
    ...
    fun hideIfRoomItems(withAnim: Boolean) {
        ...
        anims = ArrayList()
        val duration: Long = if (withAnim) HIDE_IF_ROOM_ITEMS_ANIM_DURATION.toLong() else 0.toLong()
        ...
        for (i in 0 until actionItemIndex) {
            ...
            anims.add(
                ViewPropertyObjectAnimator.animate(currentChild)
        ...
        for (i in actionItemIndex until diff + actionItemIndex) {
            ...
            if (i != childCount - 1) {
                anims.add(
                    ViewPropertyObjectAnimator.animate(currentView)
                        .setDuration(duration)
                        .addListener(object : AnimatorListenerAdapter() {
                            override fun onAnimationEnd(animation: Animator) {
        ...
        val actionItemsCount = actionItemIndex
        ...
        if (anims.isNotEmpty()) {
            ...
            animSet.addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    if (mOnVisibleWidthChangedListener != null) {
                        visibleWidth = ACTION_DIMENSION_PX.toInt() * actionItemsCount
                        mOnVisibleWidthChangedListener!!.onItemsMenuVisibleWidthChanged(visibleWidth)
                    }
                }
            })
    ...
    fun showIfRoomItems(withAnim: Boolean) {
        ...
        for (i in 0 until childCount) {
            ...
            if (i < mActionItems.size) {
                ...
                val actionItem: MenuItem = mActionItems[i]
        ...
        animSet.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                if (mOnVisibleWidthChangedListener != null) {
                    visibleWidth =
                        childCount * ACTION_DIMENSION_PX.toInt() - if (mHasOverflow) Util.dpToPx(8) else 0
                    mOnVisibleWidthChangedListener!!.onItemsMenuVisibleWidthChanged(visibleWidth)
                }
            }
        })
    ...
    private fun filter(
        target: List<MenuItemImpl>?,
        predicate: MenuItemImplPredicate
    ): List<MenuItemImpl> {
        val result: MutableList<MenuItemImpl> = ArrayList()
    ...
    fun setOnVisibleWidthChanged(listener: OnVisibleWidthChangedListener?) {
        mOnVisibleWidthChangedListener = listener
    }
