...
import com.vocaby.searchview.suggestions.SearchSuggestionsAdapter
import com.vocaby.searchview.suggestions.model.SearchSuggestion
import com.vocaby.searchview.util.view.SearchInputView
...
class SearchView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) :
    ...
    private lateinit var mQuerySection: CardView
    ...
    private lateinit var mMenuBtnDrawable: DrawerArrowDrawable
    private lateinit var mIconBackArrow: Drawable
    private lateinit var mIconSearch: Drawable

    @LeftActionMode
    var mLeftActionMode = LEFT_ACTION_MODE_NOT_SET
    private var mLeftActionIconColor = 0
    private var mSearchHint: String? = null
    private var mShowSearchKey = false
    private var mMenuOpen = false
    private lateinit var mMenuView: com.vocaby.searchview.util.view.MenuView
    private var mMenuId = -1
    private var mActionMenuItemColor = 0
    private var mOverflowIconColor = 0
    ...
    private lateinit var mSuggestionsSection: RelativeLayout
    ...
    private var mSuggestionRightIconColor = 0
    ...
    private var mSuggestionsTextSizePx = 0
    ...
    interface OnSearchListener {
         ...
         * [SearchView.setShowSearchKey]} is set to true.
         *
    ...
    private fun init(attrs: AttributeSet?) {
        mHostActivity = com.vocaby.searchview.util.Util.getHostActivity(context)
        val layoutInflater: LayoutInflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
        mMainLayout = layoutInflater.inflate(R.layout.floating_search_layout, this)
    ...
    private fun initDrawables() {
        ...
        mIconClear = com.vocaby.searchview.util.Util.getWrappedDrawable(
            context,
            R.drawable.ic_clear_black_24dp
        )
        mIconBackArrow = com.vocaby.searchview.util.Util.getWrappedDrawable(
            context,
            R.drawable.ic_arrow_back_black_24dp
        )
        mIconSearch = com.vocaby.searchview.util.Util.getWrappedDrawable(
            context,
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        if (mIsInitialLayout) {
            ...
            val addedHeight: Int = 3 * com.vocaby.searchview.util.Util.dpToPx(
            ...
            vto.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
                override fun onGlobalLayout() {
                    if (mSuggestionsSection.height == finalHeight) {
                        com.vocaby.searchview.util.Util.removeGlobalLayoutObserver(
                            mSuggestionListContainer,
                            this
                        )
    ...
    private fun applyXmlAttributes(attrs: AttributeSet) {
        ...
        try {
            val searchBarWidth: Int = a.getDimensionPixelSize(
                R.styleable.SearchView_floatingSearch_searchBarWidth,
            ...
            val searchBarLeftMargin: Int = a.getDimensionPixelSize(
                R.styleable.SearchView_floatingSearch_searchBarMarginLeft,
                ATTRS_SEARCH_BAR_MARGIN_DEFAULT
            )
            val searchBarTopMargin: Int = a.getDimensionPixelSize(
                R.styleable.SearchView_floatingSearch_searchBarMarginTop,
                ATTRS_SEARCH_BAR_MARGIN_DEFAULT
            )
            val searchBarRightMargin: Int = a.getDimensionPixelSize(
                R.styleable.SearchView_floatingSearch_searchBarMarginRight,
                ATTRS_SEARCH_BAR_MARGIN_DEFAULT
            )
            val querySectionLP: LayoutParams =
                mQuerySection.layoutParams as LayoutParams
            val dividerLP: LayoutParams =
                mDivider.layoutParams as LayoutParams
            val suggestListSectionLP: LinearLayout.LayoutParams =
                mSuggestionsSection.layoutParams as LinearLayout.LayoutParams
            val cardPadding: Int = com.vocaby.searchview.util.Util.dpToPx(
                CARD_VIEW_TOP_BOTTOM_SHADOW_HEIGHT
            )
            querySectionLP.setMargins(
                searchBarLeftMargin, searchBarTopMargin,
                searchBarRightMargin, 0
            )
            dividerLP.setMargins(
                searchBarLeftMargin + cardPadding, 0,
            ...
            setSearchHint(a.getString(R.styleable.SearchView_floatingSearch_searchHint))
            setShowSearchKey(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_showSearchKey,
                    ATTRS_SEARCH_BAR_SHOW_SEARCH_KEY_DEFAULT
                )
            )
            setCloseSearchOnKeyboardDismiss(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_close_search_on_keyboard_dismiss,
                    ATTRS_DISMISS_ON_KEYBOARD_DISMISS_DEFAULT
                )
            )
            setDismissOnOutsideClick(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_dismissOnOutsideTouch,
                    ATTRS_DISMISS_ON_OUTSIDE_TOUCH_DEFAULT
                )
            )
            setDismissFocusOnItemSelection(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_dismissFocusOnItemSelection,
                    ATTRS_DISMISS_FOCUS_ON_ITEM_SELECTION_DEFAULT
                )
            )
            mLeftActionMode = a.getInt(
                R.styleable.SearchView_floatingSearch_leftActionMode,
                ATTRS_SEARCH_BAR_LEFT_ACTION_MODE_DEFAULT
            )
            if (a.hasValue(R.styleable.SearchView_floatingSearch_menu)) {
                mMenuId = a.getResourceId(R.styleable.SearchView_floatingSearch_menu, -1)
            }
            setDimBackground(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_dimBackground,
                    ATTRS_SHOW_DIM_BACKGROUND_DEFAULT
                )
            )
            setShowMoveUpSuggestion(
                a.getBoolean(
                    R.styleable.SearchView_floatingSearch_showMoveSuggestionUp,
                    ATTRS_SHOW_MOVE_UP_SUGGESTION_DEFAULT
                )
            )
            mSuggestionSectionAnimDuration = a.getInt(
                R.styleable.SearchView_floatingSearch_suggestionsListAnimDuration,
                ATTRS_SUGGESTION_ANIM_DURATION_DEFAULT
            ).toLong()
            setBackgroundColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_backgroundColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
                        R.color.background
                    )
                )
            ...
            setLeftActionIconColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_leftActionColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
                        R.color.left_action_icon
                    )
                )
            )
            setActionMenuOverflowColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_actionMenuOverflowColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
                        R.color.overflow_icon_color
                    )
                )
            )
            setMenuItemIconColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_menuItemIconColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
                        R.color.menu_icon_color
                    )
                )
            )
            setDividerColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_dividerColor,
                    com.vocaby.searchview.util.Util.getColor(context, R.color.divider)
                )
            )
            setClearBtnColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_clearBtnColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
            ...
            val viewTextColor: Int = a.getColor(
                R.styleable.SearchView_floatingSearch_viewTextColor,
                com.vocaby.searchview.util.Util.getColor(context, R.color.dark_gray)
            )
            setViewTextColor(viewTextColor)
            setQueryTextColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_viewSearchInputTextColor,
                    viewTextColor
                )
            )
            setSuggestionsTextColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_viewSuggestionItemTextColor,
                    viewTextColor
                )
            )
            setHintTextColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_hintTextColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
                        R.color.hint_color
                    )
            ...
            setSuggestionRightIconColor(
                a.getColor(
                    R.styleable.SearchView_floatingSearch_suggestionRightIconColor,
                    com.vocaby.searchview.util.Util.getColor(
                        context,
    ...
    private fun setupQueryBar() {
        ...
        val vto: ViewTreeObserver = mQuerySection.viewTreeObserver
        vto.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                com.vocaby.searchview.util.Util.removeGlobalLayoutObserver(
                    mQuerySection,
        ...
        mMenuView.setOnVisibleWidthChanged(object :
            com.vocaby.searchview.util.view.MenuView.OnVisibleWidthChangedListener {
        ...
        mSearchInput.addTextChangedListener(object :
            com.vocaby.searchview.util.adapter.TextWatcherAdapter() {
    ...
    private fun handleOnVisibleMenuItemsWidthChanged(menuItemsWidth: Int) {
        if (menuItemsWidth == 0) {
            mClearButton.translationX =
                -com.vocaby.searchview.util.Util.dpToPx(4).toFloat()
            var paddingRight: Int = com.vocaby.searchview.util.Util.dpToPx(4)
            paddingRight += if (isSearchBarFocused) {
                com.vocaby.searchview.util.Util.dpToPx(CLEAR_BTN_WIDTH_DP)
            } else {
                com.vocaby.searchview.util.Util.dpToPx(14)
            }
            ...
            if (isSearchBarFocused) {
                paddingRight += com.vocaby.searchview.util.Util.dpToPx(CLEAR_BTN_WIDTH_DP)
            }
     ...
     * @return
     */
    val currentMenuItems: List<Any> get() = mMenuView.currentMenuItems
     ...
     * overflow icon.
     */
    private fun setActionMenuOverflowColor(color: Int) {
    ...
    fun setSuggestionRightIconColor(color: Int) {
        mSuggestionRightIconColor = color
        mSuggestionsAdapter?.setRightIconColor(mSuggestionRightIconColor)
    }
    ...
    private fun refreshLeftIcon() {
        val leftActionWidthAndMarginLeft: Int = com.vocaby.searchview.util.Util.dpToPx(
            LEFT_MENU_WIDTH_AND_MARGIN_START_DP
        )
    ...
    private fun setupSuggestionSection() {
        ...
        val gestureDetector = GestureDetector(
            context,
            object : com.vocaby.searchview.util.adapter.GestureDetectorListenerAdapter() {
                override fun onScroll(
                    ...
                    mHostActivity?.let { com.vocaby.searchview.util.Util.closeSoftKeyboard(it) }
                    return false
                }
            })

        mSuggestionsList.addOnItemTouchListener(object :
            com.vocaby.searchview.util.adapter.OnItemTouchListenerAdapter() {
        ...
        val cardViewBottomPadding: Int = com.vocaby.searchview.util.Util.dpToPx(
            CARD_VIEW_CORNERS_AND_TOP_BOTTOM_SHADOW_HEIGHT
        )
    ...
    private fun swapSuggestions(
        ...
        mSuggestionsList.viewTreeObserver
            .addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
                override fun onGlobalLayout() {
                    ...
                    val isSuggestionItemsFillRecyclerView =
                    ...
                    val suggestionsListLm: LinearLayoutManager =
                        mSuggestionsList.layoutManager as LinearLayoutManager
        ...
        mDivider.visibility =
            if (newSearchSuggestions.isNotEmpty()) View.VISIBLE else View.GONE
    }

    //returns true if the suggestion items occupy the full RecyclerView's height, false otherwise
    private fun updateSuggestionsSectionHeight(
        newSearchSuggestions: List<SearchSuggestion>,
        withAnim: Boolean
    ): Boolean {
        val cardTopBottomShadowPadding: Int = com.vocaby.searchview.util.Util.dpToPx(
            CARD_VIEW_CORNERS_AND_TOP_BOTTOM_SHADOW_HEIGHT
        )
        val cardRadiusSize: Int = com.vocaby.searchview.util.Util.dpToPx(
            CARD_VIEW_TOP_BOTTOM_SHADOW_HEIGHT
        )
        val visibleSuggestionHeight = calculateSuggestionItemsHeight(
        ...
        val addedTranslationYForShadowOffsets =
            if (diff <= cardTopBottomShadowPadding) -(cardTopBottomShadowPadding - diff) else if (diff < mSuggestionListContainer.height - cardTopBottomShadowPadding) cardRadiusSize else 0
        val newTranslationY = (-mSuggestionListContainer.height +
                visibleSuggestionHeight + addedTranslationYForShadowOffsets).toFloat()

        //todo go over
        val fullyInvisibleTranslationY =
            (-mSuggestionListContainer.height + cardRadiusSize).toFloat()
            ...
            ViewCompat.animate(mSuggestionListContainer).setInterpolator(
                ...
                .setUpdateListener { view ->
                    if (mOnSuggestionsListHeightChanged != null) {
                        val newSuggestionsHeight =
                            abs(view.translationY - fullyInvisibleTranslationY)
    ...
    private fun calculateSuggestionItemsHeight(
    ...
    private fun setSearchFocusedInternal(focused: Boolean) {
        ...
        if (focused) {
            ...
            com.vocaby.searchview.util.Util.showSoftKeyboard(context, mSearchInput)
            if (mMenuOpen) {
                closeMenu(false)
            }
            if (mIsTitleSet) {
            ...
            mClearButton.visibility = if (mSearchInput.text
                    .toString().isEmpty()
            ...
            if (mHostActivity != null) {
                com.vocaby.searchview.util.Util.closeSoftKeyboard(mHostActivity!!)
            }
            if (mIsTitleSet) {
                mSkipTextChangeEvent = true
    ...
    private fun changeIcon(imageView: ImageView, newIcon: Drawable, withAnim: Boolean) {
        ...
        if (withAnim) {
            val fadeInVoiceInputOrClear: ObjectAnimator =
    ...
    private fun transitionInLeftSection(withAnim: Boolean) {
        ...
        when (mLeftActionMode) {
            ...
            LEFT_ACTION_MODE_SHOW_SEARCH -> {
                ...
                if (withAnim) {
                    ...
                    val rotateAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).rotation(0F).get()
                    val fadeAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).alpha(1.0f).get()
            ...
            LEFT_ACTION_MODE_NO_LEFT_ACTION -> {
                mLeftAction.setImageDrawable(mIconBackArrow)
                if (withAnim) {
                    val searchInputTransXAnim: ObjectAnimator = ViewPropertyObjectAnimator
                        .animate(mSearchInputParent).translationX(0F).get()
                    mLeftAction.scaleX = 0.5f
                    mLeftAction.scaleY = 0.5f
                    mLeftAction.alpha = 0.0f
                    mLeftAction.translationX =
                        com.vocaby.searchview.util.Util.dpToPx(8).toFloat()
                    val transXArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).translationX(1.0f).get()
                    val scaleXArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).scaleX(1.0f).get()
                    val scaleYArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).scaleY(1.0f).get()
                    val fadeArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).alpha(1.0f).get()
    ...
    private fun transitionOutLeftSection(withAnim: Boolean) {
        when (mLeftActionMode) {
            ...
            LEFT_ACTION_MODE_NO_LEFT_ACTION -> {
                mLeftAction.setImageDrawable(mIconBackArrow)
                if (withAnim) {
                    val searchInputTransXAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mSearchInputParent)
                            .translationX(
                                (-com.vocaby.searchview.util.Util.dpToPx(
                                    LEFT_MENU_WIDTH_AND_MARGIN_START_DP
                                )).toFloat()
                            ).get()
                    val scaleXArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).scaleX(0.5f).get()
                    val scaleYArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).scaleY(0.5f).get()
                    val fadeArrowAnim: ObjectAnimator =
                        ViewPropertyObjectAnimator.animate(mLeftAction).alpha(0.5f).get()
    ...
    fun setOnHomeActionClickListener(listener: OnHomeActionClickListener?) {
        mOnHomeActionClickListener = listener
    }
    ...
    fun setOnMenuItemClickListener(listener: OnMenuItemClickListener?) {
        mActionMenuItemListener = listener
    ...
    private fun openMenuDrawable(drawerArrowDrawable: DrawerArrowDrawable, withAnim: Boolean) {
        if (withAnim) {
            val anim: ValueAnimator = ValueAnimator.ofFloat(0.0f, 1.0f)
    ...
    private fun closeMenuDrawable(drawerArrowDrawable: DrawerArrowDrawable, withAnim: Boolean) {
        if (withAnim) {
            val anim: ValueAnimator = ValueAnimator.ofFloat(1.0f, 0.0f)
            anim.addUpdateListener { animation ->
                val value = animation.animatedValue as Float
                drawerArrowDrawable.progress = value
            }
    ...
    override fun onSaveInstanceState(): Parcelable {
        ...
        savedState.suggestions = mSuggestionsAdapter?.dataSet ?: ArrayList()
    ...
    internal class SavedState : BaseSavedState {
        var suggestions: List<SearchSuggestion> =
            ArrayList()
        var isFocused = false
        var query: String = ""
        ...
        var actionOverflowMenuColor = 0
        var menuItemIconColor = 0
        var leftIconColor = 0
        var clearBtnColor = 0
        var suggestionUpBtnColor = 0
        var dividerColor = 0
        var menuId = 0
        var leftActionMode = 0
    ...
    private val mDrawerListener: DrawerLayout.DrawerListener = DrawerListener()
    ...
    fun detachNavigationDrawerFromMenuButton(@NonNull drawerLayout: DrawerLayout) {
    ...
    private inner class NavDrawerLeftMenuClickListener(drawerLayout: DrawerLayout) :
        OnLeftMenuClickListener {
        var mDrawerLayout: DrawerLayout = drawerLayout
